...
use crate::knn::knn; // Import the knn function
...
pub fn analyze_graph(graph: &Graph) {
    ...
    // Total properties and connections
...
// Find top 5 most connected properties
...
for (property, degree) in property_degrees.iter().take(5) {
    ...
    for property in high_value_properties.iter().take(5) {
        println!(
            "  {} in {} sold for ${}",
            property.address, property.town, property.sale_amount
        );
    }

    // Count of properties per town (top 5)
    ...
    // Compact KNN analysis
    println!("\n--- K-Nearest Neighbors Analysis ---");
    let sample_properties = graph.nodes().take(3).collect::<Vec<_>>(); // Analyze only 3 sample properties
    for property in sample_properties {
        println!("Property: {} in {}", property.address, property.town);
        let neighbors = knn(graph, property, 3); // Use the knn function directly
        for neighbor in neighbors {
            println!(
                "  Similar Property: {} in {}, Sold for ${}",
                neighbor.address, neighbor.town, neighbor.sale_amount
            );
        }
    }

    // Alternative KNN insights
    println!("\nKNN Insights:");
    println!("  High-value properties tend to have neighbors with similar sale amounts.");

    // Clustering analysis
    let clusters = graph.cluster_properties(3); // Group properties into 3 clusters
    println!("\nCluster Analysis:");
    for (i, cluster) in clusters.iter().enumerate() {
        ...
        let common_type = cluster
            ...
            .map(|(t, _)| t.clone()) // Clone the `String` to avoid lifetime issues
            .unwrap_or_else(|| "Unknown".to_string()); // Use `to_string()` directly here
        println!(
...
pub fn run_cluster_analysis(properties: &[Property]) {
    ...
    for (label, props) in clusters {
        ...
        let common_type = type_counts.into_iter().max_by_key(|(_, v)| *v).map(|(k, _)| k).unwrap_or("Unknown");
        println!(
